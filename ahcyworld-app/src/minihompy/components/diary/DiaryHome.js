...
const DiaryHome = () => {
    ...
    useEffect(() => {
        axios({
            ...
            .then((response) => {
                ...
                const formattedDates = diaries.map((diary) => moment(diary.eventDate).format("YYYY-MM-DD"));
    ...
    const handleDateClick = (date) => {
        ...
        axios
            .get(`http://localhost:8070/cyworld/cy/diaries/detail-by-date/${formattedDate}`)
            .then((response) => {
            ...
            .catch((error) => {
                ...
                setShowModal(true);
                console.error("다이어리 내용을 가져오는데 실패 했습니다.", error);
            });
    };
