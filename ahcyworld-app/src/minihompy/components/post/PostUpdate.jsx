...
const PostUpdate = () => {
    ...
    const updateSubmit = async (e) => {
        ...
        const delFileIds = originFileList.filter((item) => item.status === false).map((item) => item.id);

        console.log("delFileList: ", delFileIds);

        formData.append(`delFile`, JSON.stringify(delFileIds));

        const response = await api.put(`http://localhost:8070/${hompyId}/${postName}/${folderId}/update`, formData, {
            headers: {
    ...
    const fileAdd = () => {
        if (post.fileList.length + originFileList.length < 10) {
            setPost({
                ...post,
                fileList: [...post.fileList, { id: post.fileList.length + 1, sourceName: null }],
            });
        } else {
            ...
            <Form onSubmit={updateSubmit} encType='multipart/form-data'>
                ...
                {post?.fileList.map((item, idx) => (
                    <Form.Group key={item.id} className='files'>
                        ...
                        />
                        <Button type='button' variant='danger' name='fileList' onClick={(e) => fileDelete(e, item.id)}>
                            삭제
                        </Button>
                    </Form.Group>
                ))}
