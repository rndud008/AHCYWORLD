...
const PostDetail = ({ folderList, setMoveFolderId, moveFolderId }) => {
    ...
    const download = async (item) => {
        const response = await api.get(`http://localhost:8070/post/download?id=${item.id}`, { responseType: "blob" });
        ...
        <Container>
            <div>
                ...
                <div>
                    ...
                    <Button
                        onClick={() => navigate(`/post/${hompyId}/${postName}/${post?.folder.id}/update/${postId}`)}
                    >
                        수정
                    </Button>
