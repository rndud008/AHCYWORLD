                    ...
                    <ListGroup>
                        {updatedRequests.map((request) => (
                            <ListGroup.Item key={request.id}>
                                <div>
                                    {request.user.name}님께서 {request.friendUser.name}님과
                                    <br />
                                    일촌맺기를 희망합니다.
                                </div>
                                <div style={{ border: "3px solid black", height: "100px" }}>{request.message}</div>
                                <div>
                                    {request.user.name}({request.userName}) - {request.friendUser.name}(
                                    {request.friendName})
                                    <br />
                                    <Button onClick={() => friendShipStatus(request.id, "true")}>수락</Button>{" "}
