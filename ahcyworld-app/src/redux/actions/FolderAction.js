...
function getFolderListAxios(hompyId, postName) {
    return async (dispatch, getState) => {
        const response = await api.get(`http://localhost:8070/${hompyId}/${postName}/list`, {
            headers: {
...
function createFolderAxios(hompyId, postName, folder) {
    return async (dispatch, getState) => {
        const response = await api.post(`http://localhost:8070/${hompyId}/${postName}/write`, folder);
...
function updateFolderAxios(hompyId, postName, folder) {
    return async (dispatch, getState) => {
        const response = await api.put(`http://localhost:8070/${hompyId}/${postName}/update`, folder);
