...
function findPostAxios(hompyId, postName, folderId, postId) {
    return async (dispatch, getState) => {
        const response = await api.get(`http://localhost:8070/${hompyId}/${postName}/${folderId}/update/${postId}`, {
            headers: {
...
function movePostFolderAxios(hompyId, postName, folderId, postId, moveFolderId, navigate) {
    return async (dispatch, getState) => {
...
function deletePostAxios(hompyId, postName, folderId, postId, navigate) {
    return async (dispatch, getState) => {
        const response = await api.delete(`http://localhost:8070/${hompyId}/${postName}/${folderId}/delete/${postId}`);
...
function createPostAxios(hompyId, postName, folderId, formData, navigate) {
    return async (dispatch, getState) => {
        const response = await api.post(`http://localhost:8070/${hompyId}/${postName}/${folderId}/write`, formData, {
            headers: {
...
function detailPostAxios(hompyId, postName, folderId, postId, navigate) {
    return async (dispatch, getState) => {
        const response = await api.get(`http://localhost:8070/${hompyId}/${postName}/${folderId}/detail/${postId}`, {
            headers: {
                Authorization: `Bearer ${Cookies.get("accessToken")}`,
            },
        });
