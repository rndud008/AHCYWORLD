...
const BestItem = () => {
    ...
    useEffect(() => {
        ...
        if(userInfo && userInfo.id){
            ...
            setUserItems([]); // 로그아웃 시 userItems 초기화
        }
    ...
    const addCart = async (item) => {
        // 로그인 상태 확인
        if(!userInfo || !userInfo.id) {
            Swal.alert("로그인이 필요합니다.", "아이템을 추가하려면 로그인이 필요합니다.", "warning", () => navigate("/login"));
            return;
        }

        // 이미 장바구니 또는 보유 여부 확인
        if(userItems.includes(item.id)){
            Swal.alert("이미 보유중입니다.", "해당 아이템을 이미 갖고 있거나 장바구니에 있습니다.", "info", () => {return;});
            return;
        }
        ...
        try{
            ...
            if (response.status === 201) {
                Swal.itemconfirm("장바구니에 추가", "장바구니화면으로 이동하시겠습니까?", "success", 
